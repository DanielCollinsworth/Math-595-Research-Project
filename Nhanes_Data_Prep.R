...
get_nhanes_data <- function(nhanes_table, vars, translate = TRUE) {
  ...
  if (translate) {
    data <- nhanesTranslate(nh_table = nhanes_table, names(data), data = data)
  }
  data
}
...
cycle_L_VitD <- get_nhanes_data("VID_L", c("SEQN", "LBXVIDMS")) %>%
  rename(LBXVIDMS = LBXVIDMS) %>%
  mutate(
    VitaminD_Status = case_when(
...
cycle_L_CVD <- get_nhanes_data(
  ...
  mutate(
    CVD = ifelse(
      MCQ160B == "Yes" | MCQ160C == "Yes" | MCQ160D == "Yes" |
        MCQ160E == "Yes" | MCQ160F == "Yes",
      "Yes", "No"
    )
  ) %>%
  rename(
...
cycle_L_Depres <- get_nhanes_data(
  "DPQ_L",
  c("SEQN", "DPQ010", "DPQ020", "DPQ030", "DPQ040", "DPQ050", "DPQ060", "DPQ070", "DPQ080", "DPQ090")
) %>%
...
cycle_P_demo <- get_nhanes_data(
  "P_DEMO",
  c("SEQN", "RIAGENDR", "RIDAGEYR", "RIDRETH3", "DMDEDUC2", "INDFMPIR", "WTINTPRP", "WTMECPRP", "SDMVPSU", "SDMVSTRA")
) %>%
  rename(
    Gender        = RIAGENDR,
    Age           = RIDAGEYR,
...
data.complete$Hypertension <- ifelse(
  data.complete$SYSTOLIC_BP >= 130 | data.complete$DIASTOLIC_BP >= 80,
  "Yes", "No"
)
...
data.complete$PIR_Group <- cut(
  data.complete$Poverty_Ratio,
