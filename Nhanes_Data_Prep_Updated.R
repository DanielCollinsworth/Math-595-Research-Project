...
df1 <- nhanes_combined %>% filter(Age >= 21)
cat("[STEP 1] Age >= 21 n =", nrow(df1), "\n")


# Keep rows with complete IBI inputs; compute IBI
...
cat("[STEP 2] After IBI inputs present n =", nrow(df2), "\n")
...
# Remove "Don't know" for Diabetes if present
df3_prep <- df2 %>%
  filter(is.na(Diabetes) | Diabetes != "Don't know") %>%
  mutate(Diabetes = droplevels(Diabetes))

# Create Hypertension from BP
df3_prep <- df3_prep %>%
...
# Now do deletion ONLY across outcome + predictors
df3 <- df3_prep %>% drop_na(any_of(reg_vars))

cat("[STEP 3] After covariate + outcome complete cases n =", nrow(df3), "\n")
df3 %>% count(Cycle) %>% print()


# Factor baselines for modeling
...
df_final <- df3 %>%
  mutate(
    ...
    Education = relevel(factor(Education),      ref = "Above High School"),
    Smoking_status = relevel(factor(Smoking_status), ref = "Never Smoker"),
    Alcohol = relevel(factor(Alcohol),        ref = "Non-Drinker"),
    Diabetes = relevel(factor(Diabetes),       ref = "No"),
    Hypertension = relevel(factor(Hypertension),   ref = "No"),
    Cycle = factor(Cycle)
  )

cat("\n[FINAL] sample n =", nrow(df_final), "\n")
df_final %>% count(Cycle) %>% print()


# Keep your quartile/group features
df_final <- df_final %>%
  mutate(
    ...
    IBI_Q  = cut(IBI,
                 breaks = quantile(IBI, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
                 labels = c("Low", "Medium-Low", "Medium-High", "High"),
                 include.lowest = TRUE),
    SII_Q  = cut(SII,
                 breaks = quantile(SII, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
                 labels = c("Low", "Medium-Low", "Medium-High", "High"),
                 include.lowest = TRUE),
    AISI_Q = cut(AISI,
                 breaks = quantile(AISI, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
                 labels = c("Low", "Medium-Low", "Medium-High", "High"),
                 include.lowest = TRUE),
    SIRI_Q = cut(SIRI,
                 breaks = quantile(SIRI, probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE),
                 labels = c("Low", "Medium-Low", "Medium-High", "High"),
                 include.lowest = TRUE)
  )
...
# scale weights
# df_final <- df_final %>% mutate(WTMEC2YR = WTMEC2YR*2)  # For 2 cycles

# Save for modeling
saveRDS(df_final, file = "data_complete_cleaned.rds")
...
cat(" - Cycle L Rows: ", sum(df_final$Cycle == "L"))
cat(" - Cycle P Rows: ", sum(df_final$Cycle == "P"))
